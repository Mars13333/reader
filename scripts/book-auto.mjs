...
For every genre, build at least one complete \`contentFlow\` loop in this order: reality question → concrete scene → source-backed core case or plot → explanation → return to present-day reality → limitations. Adjacent phases may share a segment so the transitions feel natural. In \`retentionPlan.segmentBeats\`, set \`sourceAnchor\` to the new source-backed scene/case and classify \`contentLayers\` as source, analogy, commentary, or bridge. At least 60% of segments must be anchored, with no more than two unanchored segments in a row. Make the narration naturally signal what comes from the original, what is a modern analogy, and what is the channel's judgment without turning those layers into rigid blocks. Never use a disclaimer such as “不复述具体剧情、反转或结局” to avoid telling the original's essential content. Keep the scripted introduction to 30–45 seconds. The production engine adds the fixed spoken lead “大家好，今天我们讲《书名》。” during book-picker-v2; do not duplicate it in \`script.json\`. Start the first scripted sentence with the spoken hook immediately after that lead, and enter the first concrete scene no later than 45 seconds after scripted content begins. End the final narration segment with exactly: “这里是陈拾叁，陪你一起读书破万卷。”
