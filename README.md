...
需要临时指定模型或沙箱时，可追加 `--model <model>` 或 `--sandbox <mode>`。默认沿用 Codex 当前模型配置，并使用 `workspace-write`。
...
只有批准完成后，Codex 才能编写 `content/visual-plan.json`、生成分镜插画和两种发布封面所需的封面插画。封面插画只提供原创背景与主体，必须为代码排版的真实书名保留清晰区域，不得让图片模型伪造中文书名。
